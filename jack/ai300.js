...
Array.prototype.chunk = function(n) {
    //-- Chunk an array into multi parts. Each part contain n items
    var i,j;;
    var aa = [];
    for (i=0,j=this.length; i<j; i+=n) {
        aa.push ( this.slice(i,i+n) );
    }
    return aa;
}
...
Array.prototype.limit_items = function(max) {
    let array = [];

    for (let i=0; i<Math.min(max, this.length); i++) {
        array.push(this[i])
    }
    return array;
}
Array.prototype.find_object = function(f) {
    return this.find(f) || {};
}
...
String.prototype.escape_telegram    = function () {
...
String.prototype.nFormatter         = function (digits) {
	var num = this * 1;
	var si = [{
			value: 1,
			symbol: ""
...
String.prototype.extract_domains    = function () {
    let s = this.toString() || ""
    let ret = s.match(/(\w[a-z0-9-\.\+]{0,62}\.\w{2,10}\.?[\w{2,10}]?)/gi) || []
    return [...ret]
}
...
IIM = (class IIM {
    ...
    static MOUSE_CLICK (e, timeout_tag = 3) {
        var code = "CODE: ";
        code += IIM.SET_HEAD() + jsLF;
        code += IIM.SET_TIMEOUT_TAG(timeout_tag) + jsLF;
        code += IIM.SET_FRAME(e.iframe) + jsLF;
        code += `EVENT TYPE=MOUSEDOWN SELECTOR="[xid='${IIM.XID(e)}']" BUTTON=0` + jsLF;
        code += `EVENT TYPE=MOUSEUP POINT="(1,1)"` + jsLF;
        code += IIM.SET_FOOT();
...
$$$ = (class $$$ { // 2020
        ...
        do: () => {
            return new $$$()
                .try(n, i => { 
                    ...
                    process.eclick( e )
                    process.delay(0.1)
                    process.efill(e, value)
                    process.wait(x => this.$(e).val() == value, 0.1)
                    return process.go();
        ...
        do: () => {
            return new $$$()
                .try(n, i => {
                    var process = new $$$();
                    process.iim_mouseover( e );
                    if (i==1) process.eclick( e );
                    if (i==2) process.iim_click( e );
                    else process.jclick( e );
                    process.delay( delay );
                    process.reload_$();
                    if (wait != null) process.wait( wait, timeout );
                    return process.go();
    ...
    //------ log ------
    log (func) { return this._register_action({
        type: "log",
        input: Object.values(arguments),
        do: () => {
    ...
    xxx (func) { return this._register_action({
        type: "xxx",
        input: Object.values(arguments),
        do: () => {
            xxx();
...
aiObjectDB = (class aiObjectDB extends AI200 {
    constructor (name = null) {
        ...
        this.source_unique_detectors_with_hashCode  = [];
    ...
    detect_source_unique_with_hashCode (type, func) {
        if (typeof func !== 'function') return this;
        
        this.source_unique_detectors_with_hashCode.push({
    ...
    crawl ($$, crawler, crawlers) {
        var ai = this;
        if (typeof ai.func_crawling !== 'function') return this;
        console.log("%ccrawling", "color: dimgray", ai.type);
        this.crawled = ai.func_crawling.apply(ai, [$$, crawler, crawlers]);
        ...
        function recursive (items) {
            ...
            if (items._crawler == undefined) {
                if (items.source_unique != null  &&  items.source_type != null) {
                    ...
                    items._crawler  = crawlers[ items.source_type + items.source_unique ];
                    items.id        = items._crawler != null ? items._crawler.target_id * 1 : null;
                }
            }
    ...
    sync_crawlers_db (chunk) {
        var ai = this;
        console.group("sync_crawlers_db begin", ai.crawlers.map_by('source_unique').length);
        
        //-- Chunking --//
        ai.crawlers.map_by('source_unique').chunk(500).forEach((source_uniques, i) => {
            console.log('-- ' + (i+1) + '. ' + source_uniques.length + "/" + ai.crawlers.map_by('source_unique').length + ' --')
            ai.db
                .select("_crawler", "*", {
                    source_type     : ai.type,
                    source_unique   : {in: source_uniques},
                })
                // .show_last_query()
                .then(rows => {
                    // console.log(ai.type, "rows", rows);
        ...
        // if (ai.crawlers.map_by('source_unique').length > 200)  {console.log("ai.crawlers", ai.crawlers); xxx();}
    ...
    detect_new_crawlers ($$, root) {
        ...
        ai.$$_selectors.forEach(selector => {
            ...
            $$(selector.data).forEach(e => {
                ...
                if (source_unique != null  &&  source_unique.length > 0) {
                    ...
                    //console.log(this.type + ": found %c" + source_unique, "color: yellow", source_url);
                    
                    ai.crawlers.push( new_crawler );
                } else {
                    console.log(this.type + ": %cNO source_unique", "color: red");
                }
            })
            console.groupEnd();
        })
    ...
    detect_new_crawlers_with_hashCode ($$, root) {
        var ai = this;
        ...
        if (ai.source_unique_detectors_with_hashCode.length < 1) {
            console.error("No source_unique_detectors_with_hashCode !!!");
            return ai;
        }
        
        ai.$$_selectors.forEach(selector => {
            
            console.group("DETECTING TYPE: ", selector.type, ai.type)
            
            $$(selector.data).forEach(e => {
                
                var source_unique_original = null;
                var source_unique = null;
                var source_url = null;
                
                ai.source_unique_detectors_with_hashCode.filter(detector => detector.type == selector.type).forEach(detector => {
                    switch (selector.type) {
                        case 'href': 
                            source_unique_original  = detector.func.apply(ai, [e.attr('href')]);
                            source_unique           = source_unique_original != null ? source_unique_original.toString().hashCode() : null;
                            break;
                        case 'text': 
                            source_unique_original  = detector.func.apply(ai, [e.text()]);
                            source_unique           = source_unique_original != null ? source_unique_original.toString().hashCode() : null;
                            break;
                    }
                ...
                if (source_unique != null) {
                    ...
                    if (source_unique == ai.type) return;
                    if (source_unique[0] == "#") return;
                    if (ai.crawlers.map_by('source_unique').has(new_crawler.source_unique)) return;
                    
                    // console.log(this.type + ": found %c" + source_unique_original, "color: yellow", new_crawler.source_unique, source_url);
    ...
    load_crawlers_db () {
        ...
        console.log("query sql_insert_crawlers begin")
        ai.db.query( ai.sql_insert_crawlers() )
        console.log("query sql_insert_crawlers end")
        ...
        if (sqls.length) {
            ...
            console.log("%cLoad id from target_table", "color: gray", ai.target_table, cc.map_by('id'))
            var sqls = [];
        ...
        function recursive (items) {
            ...
            if (items._crawler == undefined) {
                if (items.source_unique != null  &&  items.source_type != null) {
                    ...
                    // console.log("items, ai.crawlers", items, ai.crawlers);
                    arrays_match([items], ai.crawlers
                        , (item, crawler) =>
                                (item.source_unique == crawler.source_unique  ||  item.source_unique.toString().hashCode() == crawler.source_unique) 
                ...
                if (typeof cond === 'object') {
                    ...
                    for (var field in cond) {
                        ...
                        if (typeof cond[field] === 'object') {
                            ...
                            if (value != cond_value) return ai.log(`%c- Skip add because field ${field} value`, "color: red", ai.type);
                        }
                    }
                }
